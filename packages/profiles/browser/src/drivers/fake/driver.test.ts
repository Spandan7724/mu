...
describe("scripting and failure injection", () => {
  test("an injected failure applies once and then clears", async () => {
    ...
    driver.failNext("protocol-mismatch", "update the browser bridge");
  ...
  test("connection state remains plain JSON", async () => {
    const { driver } = await connected();
    assertJsonSerializable(driver.status(), "connection state");
  });
});
